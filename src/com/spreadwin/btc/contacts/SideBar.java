...
public class SideBar extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < b.length; i++) {
			...
			paint.setColor(Color.rgb(171, 172, 174));
			...
			if (c[i].equals(".")) {			
				if (!c[i+1].equals(".")) {
					...
					canvas.drawText(c[i], width / 2-paint.measureText(c[i]) / 2, yPos-5, paint);	
				}
			}else{
				canvas.drawText(c[i], xPos, yPos, paint);				
			}
			paint.reset();// 重置画笔
		}
